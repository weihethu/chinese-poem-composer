...
public abstract class solverInterface {
	...
	public void readInput() {
		assert (PoemManager.getInstance().poems != null && PoemManager.getInstance().poems.size() > 0);
		try {
	...
	public void printResults(String resultFile) {
		...
		for (int i = 0; i < nTopic; i++) {
			...
			while (iter.hasNext()) {
				...
				wordsList.add(new WeightedWord(term, getTermProbInTopic(i, termIndex)));
			}
			...
			for (int j = 0; j <= 10; j++)
				System.out.println(wordsList.get(j).word + ":" + wordsList.get(j).w);
		}
